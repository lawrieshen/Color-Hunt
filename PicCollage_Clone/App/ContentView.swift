...
/// - `gridToLoad` carries the ``SavedGrid`` the user tapped in the Library.
...
struct ContentView: View {
    ...
    @State private var gridToLoad: SavedGrid? = nil
    @State private var photosToLoad: PhotosLoadRequest? = nil

    var body: some View {
        TabView(selection: $selectedTab) {
            CreateView(gridToLoad: $gridToLoad, selectedTab: $selectedTab, photosToLoad: $photosToLoad)
                .tabItem { Label("Create", systemImage: "plus.rectangle.on.rectangle") }
                .tag(0)

            LibraryView(gridToLoad: $gridToLoad, selectedTab: $selectedTab)
